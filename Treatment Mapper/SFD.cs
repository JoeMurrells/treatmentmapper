            ...
            try
            {
                ...
                foreach (var T in sfdtreatments)
                {
                    ...
                    Parallel.ForEach(masterlist, M =>
                    {
                        ...
                        string description = T.nomenclature.ToString();
                    ...
                    {
                        string userCode = Interaction.InputBox($"Original Description : {T.nomenclature} Best match found : {finalDesc} Match : {finalMatch}, Please confirm or enter new code.", "Confirm Code", $"{finalResult}");
                        ...
                        while (int.TryParse(userCode, out convertedCode) == false || valid_codes.Contains(Convert.ToInt32(userCode)) == false)
                        {
                            ...
                            userCode = Interaction.InputBox($"Original Description : {T.nomenclature} Best match found : {finalDesc} Match : {finalMatch}, Please confirm or enter new code.", "Confirm Code", $"{finalResult}");
                        }

                        T.dentally_code = convertedCode;
                        MASTER updateMaster = new MASTER();
                        updateMaster.UpdateMasterList(masterPath, T.nomenclature,T.dentally_code);
                    }
                    else { T.dentally_code = finalResult; }

                    log.UpdateLog(logcheck, T.nomenclature, finalDesc, finalMatch, finalResult, exePath);
                ...
                generateOutput.WriteOutputCSV(outputcsv, sfdtreatments);
