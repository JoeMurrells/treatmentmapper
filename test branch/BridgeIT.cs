            ...
            try {
                ...
                foreach (var T in bridgeTreatments)
                {
                    ...
                    Parallel.ForEach(masterlist, M =>
                    {
                        ...
                        string description = T.treatment_id.ToString();
                    ...
                    {
                        string userCode = Interaction.InputBox($"Original Description : {T.treatment_id} Best match found : {finalDesc} Match : {finalMatch}, Please confirm or enter new code.", "Confirm Code", $"{finalResult}");
                        ...
                        while (int.TryParse(userCode, out convertedCode) == false || valid_codes.Contains(Convert.ToInt32(userCode)) == false)
                        {
                            ...
                            userCode = Interaction.InputBox($"Original Description : {T.treatment_id} Best match found : {finalDesc} Match : {finalMatch}, Please confirm or enter new code.", "Confirm Code", $"{finalResult}");
                        }

                        T.dentally_code = convertedCode;
                        MASTER updateMaster = new MASTER();
                        updateMaster.UpdateMasterList(masterPath, T.treatment_id, T.dentally_code);
                    }
                    else { T.dentally_code = finalResult; }

                    log.UpdateLog(logcheck, T.treatment_id, finalDesc, finalMatch, finalResult, exePath);
                ...
                generateOutput.WriteOutputCSV(outputcsv, bridgeTreatments);
